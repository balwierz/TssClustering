...
int operator>(const heapElemT &a, const heapElemT &b);
...
template<class dataT> void updateable_heap<dataT>::downheap(int pos)
{
	register int leftKidPos = pos << 1;
	register int toChangePos = pos;
...
template<class dataT> void updateable_heap<dataT>::upheap(int pos)
{
...
template<class dataT> void updateable_heap<dataT>::update(int pos, dataT newVal)
{
	tab[pos] = newVal;
...
template<class dataT> void updateable_heap<dataT>::updated(int pos)
{
...
template<class dataT> void updateable_heap<dataT>::erase(int pos)
{
	tab[pos] = tab[s--];
	tab[pos] == pos;
	updated(pos);
